...
class HoldingPositionScoringSystem:
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS {self.REDUCTION_CHECKS_TABLE} (
                    ...
                    second_15m_open TEXT NOT NULL DEFAULT '',
                    second_15m_close TEXT NOT NULL DEFAULT '',
                    third_15m_open TEXT NOT NULL DEFAULT '',
                    third_15m_close TEXT NOT NULL DEFAULT '',
                    previous_total_score TEXT NOT NULL DEFAULT '',
                    recent_score_drawdown TEXT NOT NULL DEFAULT '',
                    latest_macd TEXT NOT NULL DEFAULT '',
                    second_macd TEXT NOT NULL DEFAULT '',
                    third_macd TEXT NOT NULL DEFAULT '',
    ...
    def _has_macd_for_positions(self, positions: list[dict[str, Any]], decision_round_ts: int) -> bool:
        """Return True only after current-round 15m MACD exists for active held symbols."""
        symbols = [self._base_symbol(str(position.get("symbol", ""))) for position in positions]
        ...
        try:
            with self._connect() as conn:
                for symbol in symbols:
                    row = conn.execute(
                        "SELECT 1 FROM macd_indicators WHERE symbol = ? AND interval = '15m' AND open_time <= ? LIMIT 1",
    ...
    def _evaluate_reduction_rules(
        ...
        recent_macds = self._recent_15m_macds(symbol, limit=3)
        ...
        latest_macd = recent_macds[0] if recent_macds else Decimal("0")
        second_macd = recent_macds[1] if len(recent_macds) > 1 else Decimal("0")
        third_macd = recent_macds[2] if len(recent_macds) > 2 else Decimal("0")
        ...
        second_kline = recent_15m_open_closes[1] if len(recent_15m_open_closes) > 1 else None
        third_kline = recent_15m_open_closes[2] if len(recent_15m_open_closes) > 2 else None
        second_open = second_kline[0] if second_kline else Decimal("0")
        second_close = second_kline[1] if second_kline else Decimal("0")
        ...
        if rule2_in_cooldown:
            ...
            reasons.append("rule2_trend_weakening")

        # 规则三已删除，不再作为减仓触发条件。保留历史字段计算仅用于兼容旧数据。
        ...
        return PositionReductionCheck(symbol, round_ts, self._fmt_decimal(highest), self._fmt_decimal(current_price), self._fmt_decimal(atr14), self._fmt_decimal(latest_ema16), self._fmt_decimal(latest_ema21), self._fmt_decimal(drawdown), self._fmt_decimal(equity), self._fmt_decimal(two_r), self._fmt_decimal(one_r), self._fmt_decimal(unrealized_pnl), open_score, latest_score, self._fmt_decimal(score_drawdown), self._fmt_decimal(latest_open), self._fmt_decimal(latest_close), self._fmt_decimal(second_open), self._fmt_decimal(second_close), self._fmt_decimal(third_open), self._fmt_decimal(third_close), previous_score, self._fmt_decimal(recent_score_drawdown), self._fmt_decimal(latest_macd), self._fmt_decimal(second_macd), self._fmt_decimal(third_macd), self._fmt_decimal(self._decimal_from(open_entry_price, Decimal("0"))) if open_entry_price else "", "+".join(matched_rules), triggered, "、".join(dict.fromkeys(tags)), reason, now_ms)
    ...
    def _latest_atr14_15m(self, symbol: str) -> Decimal:
        try:
            with self._connect() as conn:
                row = conn.execute(
                    "SELECT atr14 FROM atr_15m_indicators WHERE symbol = ? ORDER BY open_time DESC LIMIT 1",
                    (symbol,),
                ).fetchone()
        except sqlite3.Error:
            return Decimal("0")
    ...
    def _recent_15m_macds(self, symbol: str, limit: int = 3) -> list[Decimal]:
        try:
            with self._connect() as conn:
                rows = conn.execute(
                    "SELECT macd FROM macd_indicators WHERE symbol = ? AND interval = '15m' ORDER BY open_time DESC LIMIT ?",
                    (symbol, limit),
                ).fetchall()
        except sqlite3.Error:
            return []
        return [self._decimal_from(row["macd"], Decimal("0")) for row in rows]
    ...
    def _save_reduction_check(self, check: PositionReductionCheck) -> None:
        with self._connect() as conn:
            conn.execute(
                ...
                (symbol, decision_round_ts, highest_15m_high, current_price, atr14, ema16, ema21, price_drawdown_ratio, account_equity_usdt, two_r_usdt, one_r_usdt, unrealized_pnl, open_total_score, latest_total_score, score_drawdown, latest_15m_open, latest_15m_close, second_15m_open, second_15m_close, third_15m_open, third_15m_close, previous_total_score, recent_score_drawdown, latest_macd, second_macd, third_macd, open_entry_price, rule_name, triggered, tag, reason, checked_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(symbol, decision_round_ts) DO UPDATE SET
                    ...
                    second_15m_open=excluded.second_15m_open,
                    second_15m_close=excluded.second_15m_close,
                    third_15m_open=excluded.third_15m_open,
                    third_15m_close=excluded.third_15m_close,
                    previous_total_score=excluded.previous_total_score,
                    recent_score_drawdown=excluded.recent_score_drawdown,
                    latest_macd=excluded.latest_macd,
